...
> **AVISO DE ESTA COPIA (Patitascalidas, 21/09/2026).**
> Esto es una copia **solo de texto** de `pbakaus/impeccable` v4.3.1 (Apache 2.0).
> **No lleva la carpeta `scripts/` ni el binario `impeccable`.**
>
> Por qué: el lanzador original descarga un ejecutable cerrado desde las
> releases de GitHub (verificado por sha256, eso sí) y lo corre en esta
> máquina, que tiene credenciales de escritura de la tienda de verdad. Ese
> binario no se puede leer, así que no se puede auditar, y la regla de la casa
> es no dar por bueno lo que no se ha verificado. El resto del skill —que es
> casi todo: el criterio de diseño— es markdown y sí se ha leído.
>
> **Qué hacer cuando el SKILL diga de ejecutar `scripts/impeccable`:** no está.
> Sigue la propia salida del skill para ese caso («Launcher unavailable»): avisa
> en un mensaje aparte de que no se cargó el contexto, lee `PRODUCT.md` y
> `DESIGN.md` del proyecto si existen, y continúa. Las referencias de
> `reference/degraded/` están para eso.
>
> Si algún día se quiere el motor entero, se instala en la máquina de Pablo,
> donde el riesgo lo decide él, no aquí.
